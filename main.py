...
def write_log(log):
    print(log)
    with open("../dataset/log.txt", 'a', encoding='utf8') as log_f:
        log_f.write(log + "\n")

parser = argparse.ArgumentParser(description='Pedestrian Attribute Framework')
parser.add_argument('--experiment', default='foottraffic', type=str, required=True, help='(default=%(default)s)')
...
parser.add_argument('--print_freq', default=1, type=int, required=False, help='(default=%(default)d)')
parser.add_argument('--save_freq', default=1, type=int, required=False, help='(default=%(default)d)')
...
def main():
    ...
    for arg in vars(args):
        write_log('\t' + arg + ':' + str(getattr(args, arg)))
    write_log('=' * 100)
...
def validate(val_loader, model, criterion, epoch):
    ...
    write_log(' * Accu {top1.avg:.3f}'.format(top1=top1))
    return top1.avg
...
def test(test_loader, model, attr_num, description):
    ...
    write_log('=' * 100)
    write_log('\t     Attr              \tp_true/n_true\tp_tol/n_tol\tp_pred/n_pred\tcur_mA')
    mA = 0.0
    for it in range(attr_num):
        #####################################################################################
        if pos_cnt[it] == 0:
            pos_cnt[it] == 0.00000001
        if pos_tol[it] == 0:
            pos_tol[it] == 0.00000001
        if neg_cnt[it] == 0:
            neg_cnt[it] == 0.00000001
        if neg_tol[it] == 0:
            neg_tol[it] == 0.00000001
        #####################################################################################
        cur_mA = ((1.0 * pos_cnt[it] / pos_tol[it]) + (1.0 * neg_cnt[it] / neg_tol[it])) / 2.0
        mA = mA + cur_mA
        write_log('\t#{:2}: {:18}\t{:4}\{:4}\t{:4}\{:4}\t{:4}\{:4}\t{:.5f}'.format(it, description[it], pos_cnt[it],
    ...
    if attr_num != 1:
        ...
        write_log('\t' + 'Accuracy:  ' + str(accu))
        write_log('\t' + 'Precision: ' + str(prec))
        write_log('\t' + 'Recall:    ' + str(recall))
        write_log('\t' + 'F1_Score:  ' + str(f1))
    write_log('=' * 100)
...
def adjust_learning_rate(optimizer, epoch, decay_epoch):
    ...
    for epc in decay_epoch:
        if epoch >= epc:
            ...
            break
    write_log("")
    write_log('Learning Rate:' + str(lr))
    write_log("")
...
if __name__ == '__main__':
    try:
        if os.path.exists("../dataset/log.txt"):
            os.remove("../dataset/log.txt")
        now = datetime.datetime.now()
        write_log("start_learning : " + str(now))
        write_log("cmd : python " + " ".join(sys.argv))
        main()
    except KeyboardInterrupt:
        now = datetime.datetime.now()
        write_log("finish_learning : " + str(now))
